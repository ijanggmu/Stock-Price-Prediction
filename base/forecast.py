...
df=pd.read_csv('data/forward.csv', encoding = 'unicode_escape')
...
df_scaled = scaler.fit_transform(dtp)#dfs
forecast_features_set = [] #features set
labels = [] 
...
forecast_features_set.shape #features_set
model = Sequential()
...
df_predict=pd.DataFrame(scaler.inverse_transform(forecast_list),index=dtp[-forecast_features_set.shape[1]:].index, 
                        columns=["prediction"])
